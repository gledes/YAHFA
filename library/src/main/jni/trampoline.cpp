...
#if defined(__arm__)
    memcpy((char *)targetAddr+8, &hookMethod, pointer_size);

#elif defined(__aarch64__)
    memcpy((char *)targetAddr + 12, &hookMethod, pointer_size);
#endif
...
int doInitHookCap(unsigned int cap) {
    ...
    unsigned char *buf = (unsigned char *)mmap(NULL, allSize, PROT_READ | PROT_WRITE | PROT_EXEC,
